#!/bin/bash
# ChartQA 数据集 Parquet 处理脚本
# 用于将 ChartQA 数据集的 parquet 文件转换为 JSON 和图像文件

set -e

echo "🚀 ChartQA 数据集处理启动"
echo ""
...
# Parquet 数据目录
PARQUET_DIR="${CHARTQA_ROOT}/data"

# 图像输出目录
IMAGES_DIR="${CHARTQA_ROOT}/images"

# JSON 输出目录
OUTPUT_DIR="${CHARTQA_ROOT}"

# 处理脚本路径
PROCESS_SCRIPT="${SCRIPT_DIR}/process_chartqa.py"
...
echo "============================================================"
echo "📦 ChartQA 数据集处理配置"
echo "============================================================"
echo "📁 项目目录: ${PROJECT_DIR}"
echo "📁 ChartQA 根目录: ${CHARTQA_ROOT}"
...
echo ""
echo "  验证集文件："
ls -lh "${PARQUET_DIR}"/val-*.parquet 2>/dev/null | awk '{print "    " $NF " (" $5 ")"}' || echo "    (未找到验证集 parquet 文件)"
echo ""
echo "  测试集文件："
...
for SPLIT in train val test; do
    JSON_FILE="${OUTPUT_DIR}/chartqa_${SPLIT}.json"
